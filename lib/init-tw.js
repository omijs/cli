...
function init(args) {
  ...
  var tpl = join(__dirname, "../template/tailwind");
    ...
    vfs
      ...
      .on("end", function () {
        try {
          ...
          if (existsSync(join(dest, "prettierrc"))) {
            info("Rename", "prettierrc -> .prettierrc");
            renameSync(join(dest, "prettierrc"), join(dest, ".prettierrc"));
          }
          if (customPrjName) {
            try {
